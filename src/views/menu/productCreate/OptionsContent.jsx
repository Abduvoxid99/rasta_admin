...
export default function OptionsContent({
  ...
  const onModalSubmitIngredients = (values) => {
    values.ingredients.forEach((item) => {
      handleAddOption({ ...item, is_default: false })
    })
  }
  console.log(options)
  const handleAddOption = ({ name, price, is_default = true }) => {
    let checkIsDefault = false
    options.forEach((data) => {
      if (data.child_options) {
        const is_default = data.child_options.find(
          (item) => item.is_default && item.is_required
        )
        console.log(is_default)
        if (is_default && is_default.is_default && !checkIsDefault) {
          checkIsDefault = is_default.is_default
        }
      }
    })

    console.log("checkIsDefault==>", checkIsDefault)
              ...
              ? {
                  ...
                  child_options: [
                    ...elm.child_options,
                    {
                      ...
                      is_default: optionModal ? !checkIsDefault : false,
                    },
                  ],
                }
              : elm
        ...
        prev.map((elm, i) =>
          i === parentIndex
            ...
            : {
                ...elm,
                is_default: false,
                child_options: elm.child_options.map((el) => ({
                  ...el,
                  is_default: false,
                })),
              }
          ...
          <IconButton
            ...
            onClick={() =>
              setIngredientModal({
                ...
                parentIndex: index,
                initialValues: {
                  ingredients: element.child_options.filter(
                    (elm) => !elm.is_required
                  ),
                },
              })
