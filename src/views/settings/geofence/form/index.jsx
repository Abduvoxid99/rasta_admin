  ...
  const getItem = () => {
    ...
    getOneRegion(region_id)
      .then((res) => {
        ...
        formik.setFieldValue(
          "waveTime",
          res.wave_info ? res.wave_info[0].wave_time : 0
        )
      })
    ...
    getOneGeozone(geozone_id)
      .then((res) => {
        ...
        const lats = res.points.map((item) => item.lat)
        const lons = res.points.map((item) => item.lon)
        setMapCenter([
    ...
    try {
      if (region_id && geozone_id) {
        await updateRegion(region_id, {
          ...
          name: values.name,
          wave_info: waves.map((item) => ({
            ...item,
            waveTime: values.waveTime,
          })),
        })
    ...
    <div className="w-full">
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-2 p-4 gap-4">
          ...
          <div>
            <Card title="Настройки волны">
              <div>
                <div className="flex mb-4">
                  <div className="grid grid-cols-3 w-11/12 gap-2">
                    <div className="input-label">Радиус</div>
                    <div className="input-label">Продолжительность</div>
                          ...
                          <div className="w-11/12">
                            <div className="grid grid-cols-3 gap-3">
                              ...
                              <div className="w-full">
                                ...
                                >
                                  {errors?.waveTime && touched?.waveTime
                                    ? errors?.waveTime
                                    : ""}
                                </div>
                              </div>
